...
#include FTS_UPGRADE_SKYW_FILE
};

const u8 up_fw_kernel_hlt[] = {
	...
	for (retry = 5; retry > 0; retry--) {
		...
		for (list = 0; list < (sizeof(trim_id_table) / sizeof(struct nvt_ts_trim_id_table)); list++) {
			...
			if (found_nvt_chip) {
				...
				NVT_LOG("This is NVT touch IC11  \n");
				ts->mmap = trim_id_table[list].mmap;
				NVT_LOG("This is NVT touch IC12  \n");
...
static void nvt_tp_usb_notifier_init(void)
{
	...
	res = usb_register_client(&ts->notifier_tp_usb);
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == MSM_DRM_EARLY_EVENT_BLANK) {
		...
		if (*blank == MSM_DRM_BLANK_POWERDOWN) {
			NVT_LOG("%s : MSM_DRM_BLANK_POWERDOWN nvt_ts_suspend\n",__func__);
		...
		if (*blank == MSM_DRM_BLANK_UNBLANK) {
			NVT_LOG("%s : MSM_DRM_BLANK_UNBLANK nvt_ts_resume\n",__func__);
